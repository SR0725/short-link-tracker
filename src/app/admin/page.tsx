...
import { Plus, ExternalLink, BarChart, Copy, LogOut, Settings, ChevronUp, ChevronDown, QrCode, Search, Filter } from 'lucide-react'
...
export default function AdminPage() {
  ...
  const getTagVariant = (tag: string | undefined): "default" | "secondary" | "destructive" | "outline" => {
    if (!tag) return "outline"
    
    const tagColors: Record<string, "default" | "secondary" | "destructive" | "outline"> = {
      '工作': "default",
      '個人': "secondary", 
      '行銷': "destructive",
      '社群媒體': "outline",
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              ...
              <div className="relative">
                ...
                >
                  ...
                  欄位顯示
                  <ChevronDown className={`w-4 h-4 transition-transform ${showColumnOptions ? 'rotate-180' : ''}`} />
                </Button>
                ...
                {showColumnOptions && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-800 border rounded-md shadow-lg z-10">
                    <div className="p-3 space-y-3">
                      <div className="text-sm font-medium text-gray-700 dark:text-gray-200">顯示欄位</div>
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-title"
                          ...
                          />
                          <Label htmlFor="col-title" className="text-sm">標題/名稱</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-shortUrl"
                            checked={columnVisibility.shortUrl}
                            onCheckedChange={() => toggleColumnVisibility('shortUrl')}
                          />
                          <Label htmlFor="col-shortUrl" className="text-sm">短網址</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-targetUrl"
                            checked={columnVisibility.targetUrl}
                            onCheckedChange={() => toggleColumnVisibility('targetUrl')}
                          />
                          <Label htmlFor="col-targetUrl" className="text-sm">目標網址</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-clickCount"
                            checked={columnVisibility.clickCount}
                            onCheckedChange={() => toggleColumnVisibility('clickCount')}
                          />
                          <Label htmlFor="col-clickCount" className="text-sm">點擊數/上限</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-expiresAt"
                            checked={columnVisibility.expiresAt}
                            onCheckedChange={() => toggleColumnVisibility('expiresAt')}
                          />
                          <Label htmlFor="col-expiresAt" className="text-sm">到期日</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-lastClickAt"
                            checked={columnVisibility.lastClickAt}
                            onCheckedChange={() => toggleColumnVisibility('lastClickAt')}
                          />
                          <Label htmlFor="col-lastClickAt" className="text-sm">最近點擊</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="col-createdAt"
                          ...
                          />
                          <Label htmlFor="col-createdAt" className="text-sm">建立時間</Label>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {columnVisibility.shortUrl && <TableHead>短網址</TableHead>}
                        {columnVisibility.targetUrl && <TableHead>目標網址</TableHead>}
                        ...
                        {columnVisibility.actions && <TableHead>操作</TableHead>}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                            ...
                            <TableCell>
                              <div className="max-w-xs">
                                <div className="font-medium truncate flex items-center gap-2">
                                  ...
                                  {link.tag && (
                                    <Badge variant={getTagVariant(link.tag)} className="text-xs">
                                      {link.tag}
                                    </Badge>
                                  )}
                                </div>
                ...
                {showQrCode && (
                  <div className="mt-4">
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border">
                      ...
                      <QRCodeGenerator 
                        url={`${baseUrl}/${filteredLinks.find(l => l.id === showQrCode)?.slug}`}
                      />
                    </div>
                  </div>
                )}
